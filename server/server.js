...
app.get("/api/filters", async (req, res) => {
  try {
    const products = await Product.find(
      {},
      "fragrance festival theme weight price category",
    );
...
app.post("/api/products", auth, upload.array("images", 5), async (req, res) => {
  ...
  try {
    ...
    if (req.files && req.files.length > 0) {
      // Check if user is AuraUser (team member) - save locally
      // Try to find by ID first
      let isAuraUser = await AuraUser.findById(req.user.id);

      // If not found by ID, try to find by email
      if (!isAuraUser && req.user.email) {
        isAuraUser = await AuraUser.findOne({ email: req.user.email });
      }

      if (isAuraUser) {
        // For Aura users, save files locally instead of uploading to Cloudinary
        ...
        imageUrls = await Promise.all(
          req.files.map(async (file) => {
            ...
            // Return the URL path (relative to the static file serving)
            return `/uploads/products/${filename}`;
          }),
        );
      } else {
        // Upload images to Cloudinary for regular users
  ...
  async (req, res) => {
    try {
      ...
      if (req.file) {
        ...
        const cloudinaryResult = await uploadSingleImage(
          req.file,
          "custom-orders",
        );
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (req.file) {
        ...
        if (isAuraUser) {
          // For Aura users, save file locally instead of uploading to Cloudinary
          const fs = await import("fs");
          const path = await import("path");
          ...
          // Upload image to Cloudinary for regular users
          const cloudinaryResult = await uploadSingleImage(
            req.file,
            "testimonials",
          );
...
app.post(
  ...
  async (req, res) => {
    console.log(
      "Received authenticated request to add product review:",
      req.body,
    );
    try {
      ...
      if (req.files.image && req.files.image[0]) {
        if (isAuraUser) {
          ...
          const uniqueSuffix =
            Date.now() + "-" + Math.round(Math.random() * 1e9);
          const filename = `${uniqueSuffix}-${req.files.image[0].originalname}`;
          const filePath = path.join(uploadDir, filename);

          // Write file to disk
          fs.writeFileSync(filePath, req.files.image[0].buffer);

          // Return the URL path (relative to the static file serving)
          profileImagePath = `/uploads/reviews/${filename}`;
        } else {
          // Upload image to Cloudinary for regular users
          const cloudinaryResult = await uploadSingleImage(
            req.files.image[0],
      ...
      if (req.files.images && req.files.images.length > 0) {
        if (isAuraUser) {
          ...
          productImagePaths = cloudinaryResults.map(
      ...
      if (productId) {
        ...
        if (product) {
          ...
          await TrendingProduct.updateMany(
            { productId },
            {
              $inc: { reviews: 1 },
              $set: { rating: newAvgRating },
...
app.post(
  ...
  async (req, res) => {
    console.log(
      "Received authenticated request to add featured collection:",
      req.body,
    );
    ...
    try {
      ...
      res
        .status(201)
        .json({ message: "Featured collection added successfully" });
    } catch (error) {
...
app.post(
  ...
  async (req, res) => {
    console.log(
      "Received authenticated request to add trending product:",
      req.body,
    );
    try {
...
app.delete("/api/trending-products/:id", auth, async (req, res) => {
  try {
    ...
    if (trendingProduct.uploadedBy.toString() !== req.user.id) {
      return res
        .status(403)
        .json({ message: "Not authorized to delete this trending product" });
    }
...
app.post("/api/team/signup", async (req, res) => {
  ...
  try {
    ...
    const result = await AuraUser.create({
      name,
      email,
